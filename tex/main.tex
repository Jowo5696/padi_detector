...
Both, the distribution in x and in y direction, contains additionaly to the gaussian curve other peaks in the foothills of that distribution, which is observed for all recorded runs.
By cutting away events of large charge deposition, these artifacts disappear. (Begründung)

The data, filtered as described, is shown for alumnium in \autoref{fig:2alu}.
\begin{figure}
  ...
  \label{fig:2alu}
\end{figure}
At one specific point i.e one specific strip in the detector, no event seems to be counted, which is observed for all runs.
(Begründung -> Jonas)
