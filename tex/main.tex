...
Both, the distribution in x and in y direction, contains additionaly to the gaussian curve other peaks in the foothills of the distribution, which is also observed for all recorded runs.
By cutting away events of large charge deposition, these artifacts disappear. A possible reason is, that the detector is more sensitive at these strips which leads to more counted events.
Additionally one observe, that at one specific point i.e one specific strip in the detector, no event seems to be counted. (Begründung -> Jonas)
...
Additionally on the continuous side of the gaussian distribution, another little unevenness is observed.
I could be the continuation of the gaussian curve after the sharp edge, so instead of the differences beween the two APV's, in the middle of the distribution one counts too many events. (Begründung)
...
As shown in \autoref{fig:correlation}.
